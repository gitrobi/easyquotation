...
class Leverfun:
    ...
    async def get_stock_detail(self, stock_code):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            #ws = await session.ws_connect(url, ssl=ssl_ctx)
            r_json = await session.get(Leverfun.stock_api, ssl=ssl_ctx, params=params)
            self.stocks_dict[stock_code] = Leverfun.format_response_data(r_json)
        # async with aiohttp.request('GET', Leverfun.stock_api, params=params) as r: # async with aiohttp.get(Leverfun.stock_api, params=params) as r
        #     r_json = await r.json()
        #     self.stocks_dict[stock_code] = Leverfun.format_response_data(r_json)
